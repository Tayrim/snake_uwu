...
// Кнопка, ведущая к показу рекламы — отдельный стиль, чтобы игрок
// заранее понимал, что за этим действием будет ролик.
function drawAdBtn(r, label) {
  const hover = inRect(mouse, r);
  ctx.fillStyle = hover ? '#fde047' : C.gold;
  ctx.shadowColor = 'rgba(250,204,21,0.6)';
  ctx.shadowBlur = hover ? 16 : 8;
  rr(r.x, r.y, r.w, r.h, 8); ctx.fill();
  ctx.shadowBlur = 0;
  text(label, r.x + r.w / 2, r.y + (r.h - 20) / 2, 20, '#1e293b', 'center');
}
...
function drawHUD(now) {
  ...
  if (mode === 'level') {
    ...
    if (invertedUntil > now) {
      text('☠️ Управление перепутано!', WIDTH / 2, 40, 18, C.poison, 'center');
    }
    text('Время: ' + mm + ':' + ss, 20, HEIGHT - 40, 24, C.text);
...
function drawWheel() {
  ...
  if (wheelSpin) {
    text('Крутится...', WIDTH / 2, 485, 20, C.gold, 'center');
  } else if (wheelResult) {
    const p = wheelResult;
    text(p.t === 'buff' ? '🎉 Приз: ×2 на следующую игру!' : '🎉 Приз: ' + p.v + ' монет!', WIDTH / 2, 485, 18, C.gold, 'center');
    drawBtn(BTN_SPIN, 'Готово', 20);
  } else if (canSpinWheel()) {
    drawBtn(BTN_SPIN, 'КРУТИТЬ!', 22);
  } else if (canAdSpinWheel()) {
    text('Бесплатный спин уже использован сегодня', WIDTH / 2, 485, 13, C.gray, 'center');
    drawAdBtn(BTN_SPIN_AD, '🎬 Ещё спин (' + (WHEEL_AD_SPINS_MAX - wheelExtraUsed) + ' ост.)');
  } else {
    text('Приходи завтра!', WIDTH / 2, 490, 18, C.gray, 'center');
  }
}