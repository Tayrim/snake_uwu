function handleTap(p) {
  ...
  if (state === 'menu') {
    ...
    if (inRect(p, BTN_MENU_ADV)) { startGame('adventure'); return; }
    ...
    if (canAdSpinWheel() && inRect(p, BTN_SPIN_AD) && !wheelSpin && !wheelResult) {
      requestAdSpin();
      return;
    }
  } else if (state === 'play') {
    ...
    if (canOfferDoubleLevelReward() && inRect(p, BTN_LW_X2)) {
      requestDoubleLevelReward();
      return;
    }
    ...
    if (canOfferRevive() && inRect(p, BTN_GO_REVIVE)) {
      reviveWithAd();
      return;
    }
    if (canOfferDoubleCoins() && inRect(p, BTN_GO_X2)) {
      requestDoubleCoins();
      return;
    }
    if (inRect(p, BTN_GO_RESTART)) {
      finalizeGameOverCommit();
...
function setupInput() {
  ...
  document.getElementById('fundsAd').addEventListener('click', () => requestFundsAd());
