...
function commitScore(withCoins = true, mult = 1) {
  if (score > high) { high = score; saveHigh(high); }
  logScore(score, mode);
  if (withCoins) {
    const modeMult = mode === 'adventure' ? ADV_COIN_MULT : 1;
    addCoins(coinsFromScore(score) * modeMult * mult);
  }
}
...
function startGame(m) {
  ...
  pendingGameOverCommit = null;
  gameSpeed = m === 'hard' ? SPEED_HARD : m === 'adventure' ? SPEED_ADVENTURE : SPEED_CLASSIC;
  ...
  advObstacleQueue = [];
  advObstacleCooldown = ADV_OBSTACLE_START_MS;
  door = null;
  spawnFood();
...
function failLevel(now) {
  lives = 0;
  goToGameOver(now, false);
}

// ============================================
// РЕВАНШ ЗА РЕКЛАМУ
// ============================================
// Общая точка выхода на экран проигрыша (и в классике/харде, и в уровнях).
// Если реванш ещё не использован в этой попытке — результат пока НЕ
// коммитится (очки/монеты), а игроку предлагается продолжить за рекламу.
// Коммит произойдёт либо при отказе (кнопки "Заново"/"Меню"), либо сразу,
// если реванш уже был использован.
function goToGameOver(now, withCoins) {
  frozenSec = elapsedSec(now);
  state = 'gameover';
  SFX.die();
  startMusic('menu');
  if (canOfferRevive()) {
    pendingGameOverCommit = { withCoins };
  } else {
    commitScore(withCoins);
    pendingGameOverCommit = null;
  }
}
...
function finalizeGameOverCommit() {
  if (pendingGameOverCommit) {
    commitScore(pendingGameOverCommit.withCoins, doubleCoinsUsed ? 2 : 1);
    pendingGameOverCommit = null;
  }
}

function reviveWithAd() {
  if (!canOfferRevive() || state !== 'gameover') return;
  SFX.click();
  showRewardedAd({
    onReward: () => {
      reviveUsed = true;
    ...
    // onClose без onReward (ролик закрыли раньше времени) — просто
    // остаёмся на экране gameover, ничего не меняется.
  });
}

// Удвоение монет: для classic/hard/adventure — до коммита счёта на экране gameover.
function canOfferDoubleCoins() {
  return !doubleCoinsUsed && mode !== 'level' && pendingGameOverCommit && pendingGameOverCommit.withCoins && score > 0;
}
function requestDoubleCoins() {
  if (!canOfferDoubleCoins()) return;
  SFX.click();
  showRewardedAd({
    onReward: () => { doubleCoinsUsed = true; }
  });
}

// Удвоение награды за уровень — монеты там начисляются сразу в completeLevel(),
// поэтому "удвоение" — это начисление такой же суммы ещё раз.
function canOfferDoubleLevelReward() {
  return !doubleCoinsUsed && lastLevelResult && lastLevelResult.base > 0;
}
function requestDoubleLevelReward() {
  if (!canOfferDoubleLevelReward()) return;
  SFX.click();
  showRewardedAd({
    onReward: () => {
      doubleCoinsUsed = true;
      const bonus = addCoins(lastLevelResult.base);
      lastLevelResult.reward += bonus;
    }
  });
}

function doRevive() {
  pendingGameOverCommit = null; // реванш состоялся — старый результат не коммитим
  if (mode === 'level') lives = 1;
  if (mode === 'adventure') {
    // Немного расчищаем поле, чтобы не возродиться в смертельной ловушке
    walls = []; wallsSet = new Set(); advObstacleQueue = [];
    advObstacleCooldown = ADV_OBSTACLE_START_MS;
    poison = null;
  }
  const c = { x: Math.floor(COLS / 2) * GRID, y: Math.floor(ROWS / 2) * GRID };
  snake = [c];
  dir = { x: 0, y: -GRID };
  dirQueue = [];
  growPending = false;
  particles = [];
  spawnParticles(c.x, c.y, C.ok, 20);
  quickPaused = false;
  lastStep = performance.now();
  state = 'play';
  SFX.start();
...
function update(now, dt) {
  ...
  if (state === 'play' && !quickPaused) {
    ...
    // Отравленное яблоко (уровни + приключение)
    if (mode === 'level' || mode === 'adventure') {
      if (!poison) {
        poisonCooldown -= dt;
        if (poisonCooldown <= 0) {
          const p = randomFreeCell();
    ...
    // Приключение: препятствия появляются на поле, чем дальше — тем чаще
    if (mode === 'adventure') {
      advObstacleCooldown -= dt;
      if (advObstacleCooldown <= 0) {
        const p = randomFreeCell();
        if (p) {
          const k = wallKey(p.x, p.y);
          walls.push(p);
          wallsSet.add(k);
          advObstacleQueue.push(k);
          if (advObstacleQueue.length > ADV_OBSTACLE_MAX) {
            const old = advObstacleQueue.shift();
            wallsSet.delete(old);
            walls = walls.filter(w => wallKey(w.x, w.y) !== old);
          }
        }
        advObstacleCooldown = Math.max(ADV_OBSTACLE_MIN_MS, ADV_OBSTACLE_START_MS - score * 15);
      }
    }
