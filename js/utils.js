...
function modeLabel(m) { return m === 'hard' ? 'Хардкор' : m === 'level' ? 'Уровни' : m === 'adventure' ? 'Приключение' : 'Классика'; }
...
function canAdSpinWheel() {
  return wheelLastDate === todayStr() && !wheelSpin && !wheelResult && wheelExtraUsed < WHEEL_AD_SPINS_MAX;
}

function calcBtns() {
  const w = 300, h = 50;
  const cx = WIDTH / 2 - w / 2;
  BTN_MENU_1 = { x: 205, y: 182, w: 190, h: 56 };
  BTN_MENU_ADV = { x: 205, y: 252, w: 190, h: 56 };
  BTN_MENU_2 = { x: 205, y: 322, w: 190, h: 56 };
  BTN_MENU_3 = { x: 205, y: 392, w: 190, h: 56 };
  ...
  BTN_SPIN = { x: WIDTH / 2 - 70, y: 515, w: 140, h: 50 };
  BTN_SPIN_AD = { x: WIDTH / 2 - 110, y: 517, w: 220, h: 46 };
