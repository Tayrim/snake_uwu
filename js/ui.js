...
function draw(now) {
  ...
  if (state === 'menu') {
    ...
    // Режимы в центре
    drawModeBtn(BTN_MENU_1, 'Классика', '🐍', '#166534', '#15803d', '#22c55e');
    drawModeBtn(BTN_MENU_ADV, 'Приключение', '🧭', '#7c2d12', '#9a3412', '#fb923c');
    drawModeBtn(BTN_MENU_2, 'Хардкор', '🔥', '#991b1b', '#b91c1c', '#ef4444');
    drawModeBtn(BTN_MENU_3, 'Уровни', '🧱', '#854d0e', '#a16207', '#facc15');
    ...
    if (!IS_TOUCH) {
      ...
      else if (inRect(mouse, BTN_MENU_ADV)) hovId = 'madv';
    ...
    if (state === 'countdown') {
      ...
      ctx.fillStyle = C.dim; ctx.fillRect(0, 0, WIDTH, HEIGHT);
      text('УРОВЕНЬ ' + lvlIndex + ' ПРОЙДЕН!', WIDTH / 2, 110, 42, C.accent, 'center');
      const r = lastLevelResult || { time: 0, reward: 0, newBest: false, granted: null };
      text('Время: ' + fmtTime(r.time), WIDTH / 2, 175, 24, C.text, 'center');
      if (r.newBest) text('🏆 Новый рекорд времени!', WIDTH / 2, 206, 16, C.gold, 'center');
      text(r.reward > 0 ? '+' + r.reward + ' монет (150 + очки' + (currentSkinMult() > 1 ? ' × ' + currentSkinMult() : '') + (coinBuff > 1 ? ' × 2' : '') + ')' : 'Награда: пройди быстрее своего времени', WIDTH / 2, 230, 14, r.reward > 0 ? C.gold : C.gray, 'center');
      if (canOfferDoubleLevelReward()) {
        drawAdBtn(BTN_LW_X2, '🎬 Удвоить монеты');
      } else if (r.granted) {
        text('🎁 Получен скин: ' + r.granted + '!', WIDTH / 2, 258, 15, C.ok, 'center');
      }
      ...
      ctx.fillStyle = C.dim; ctx.fillRect(0, 0, WIDTH, HEIGHT);
      text('ИГРА ОКОНЧЕНА', WIDTH / 2, 90, 50, C.accent, 'center');
      text('Очки: ' + score, WIDTH / 2, 175, 26, C.text, 'center');
      text('Рекорд: ' + high, WIDTH / 2, 210, 20, C.gold, 'center');
      const extra = mode === 'level' ? 'Уровень ' + lvlIndex + ' | ' : '';
      if (mode === 'level') {
        text(extra + '❤️ не хватило', WIDTH / 2, 240, 16, C.gray, 'center');
      } else {
        const mult = (mode === 'adventure' ? ADV_COIN_MULT : 1) * currentSkinMult() * coinBuff * (doubleCoinsUsed ? 2 : 1);
        text(extra + '+' + Math.round(coinsFromScore(score) * mult) + ' монет' + (doubleCoinsUsed ? ' (x2!)' : ''), WIDTH / 2, 240, 16, C.gray, 'center');
      }
      if (canOfferDoubleCoins()) {
        drawAdBtn(BTN_GO_X2, '🎬 Удвоить монеты');
      }
      if (canOfferRevive()) {
        drawAdBtn(BTN_GO_REVIVE, '🎬 Продолжить за рекламу');
      }
  ...
  if (!IS_TOUCH) {
    if (state === 'menu') {
      hover = inRect(mouse, BTN_MENU_1) || inRect(mouse, BTN_MENU_ADV) || inRect(mouse, BTN_MENU_2) || inRect(mouse, BTN_MENU_3) ||
      ...
      hover = inRect(mouse, BTN_BACK) || (!wheelSpin && inRect(mouse, BTN_SPIN)) ||
              (!wheelSpin && !wheelResult && canAdSpinWheel() && inRect(mouse, BTN_SPIN_AD));
    } else if (state === 'pausemenu') {
      ...
      hover = inRect(mouse, BTN_LW_NEXT) || inRect(mouse, BTN_LW_RETRY) || inRect(mouse, BTN_LW_LIST) ||
              (canOfferDoubleLevelReward() && inRect(mouse, BTN_LW_X2));
    } else if (state === 'gameover') {
      hover = inRect(mouse, BTN_GO_RESTART) || inRect(mouse, BTN_GO_MENU) ||
              (canOfferRevive() && inRect(mouse, BTN_GO_REVIVE)) ||
              (canOfferDoubleCoins() && inRect(mouse, BTN_GO_X2));
    }
  }
...
function refreshFundsText() {
  if (!fundsItem) return;
  const lack = fundsItem.price - coins;
  if (lack <= 0) {
    fundsTextEl.innerHTML = '<span class="bal">Теперь монет хватает: ' + coins + '! Загляни в магазин 🛒</span>';
  } else {
    fundsTextEl.innerHTML =
      'Для покупки <b>' + fundsItem.name + '</b> нужно <span class="price">' + fundsItem.price + ' монет</span>.<br>' +
      '<span class="bal">У тебя сейчас: ' + coins + ' монет</span><br>' +
      '<span class="lack">Не хватает: ' + lack + ' монет</span>';
  }
}

function requestFundsAd() {
  if (!fundsItem) return;
  SFX.click();
  showRewardedAd({
    onReward: () => {
      const bonus = Math.min(300, Math.ceil(fundsItem.price * 0.15));
      coins += bonus; saveCoins();
      SFX.buy();
      refreshFundsText();
    }
  });
